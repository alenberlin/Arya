...
//! their files. Removal when the whole note is deleted is driven from
//! [`crate::notes::delete_note`] via [`remove_files_for_note`]; the rows
//! themselves cascade on the note's foreign key.
...
/// Remove every attachment file for a note (the rows cascade on note delete).
pub async fn remove_files_for_note(pool: &SqlitePool, note_id: &str) -> Result<(), String> {
    let paths: Vec<String> =
        sqlx::query_scalar("SELECT path FROM note_attachments WHERE note_id = ?1")
            .bind(note_id)
            .fetch_all(pool)
            .await
            .map_err(|e| e.to_string())?;
    for path in paths {
        ...
        if let Some(dir) = path.parent() {
            // Removes the note's attachment directory once it's empty.
...
mod tests {
    ...
    #[tokio::test]
    async fn note_delete_cascades_rows_and_removes_files() {
        let pool = test_pool().await;
        let note = insert_note(&pool, "n").await.unwrap();
        let tmp = std::env::temp_dir().join(format!("arya-att-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&tmp).unwrap();
        ...
        // Mirror delete_note: remove files, then delete the note (rows cascade).
        remove_files_for_note(&pool, &note.id).await.unwrap();
        ...
        assert!(!Path::new(&att.path).exists(), "attachment file removed");
        assert!(
            list_for_note(&pool, &note.id).await.unwrap().is_empty(),
            "rows cascaded with the note"
        );
        std::fs::remove_dir_all(&tmp).ok();
    }
}
