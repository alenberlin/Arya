...
| M7 Agent runtime core | done | `make verify` exit 0 (52 Rust + 10 sidecar + 9 frontend tests, incl. the Seatbelt AC test: writes outside jail FAIL, inside SUCCEED). In-app E2E on local qwen3.6:35b (Ollama, zero cloud): session → approval-gated run_command → write_file → answer; reasoning + tools persisted to SQLite; result.md written inside the jailed workspace. Sidecar: TS on AI SDK v7, providers anthropic/openai/ollama behind one interface, approval broker (once/session/deny), steering via prepareStep, JSON-RPC over stdio. Commit follows |
